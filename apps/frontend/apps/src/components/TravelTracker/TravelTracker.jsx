...
function TravelTracker(props) {
  ...
  useEffect(() => {
    if (props.data !== "" && !isDataSet) {
      props.data.then((countries) => {
        if (countries !== "") {
          //prevents crashing if server is inaccessible
          const tempCountries = countries.data.map(
            (dbRow) => dbRow.countryCode
          );
  ...
  const handleSubmit = (event) => {
    ...
    if (
      !validCountryCodes.includes(
        document.getElementsByName("countryCode")[0].value.toUpperCase()
      )
    ) {
      event.preventDefault();
      event.target.countryCode.value = "";
      event.target.countryCode.placeholder = "Invalid country code!";
    }
  };
